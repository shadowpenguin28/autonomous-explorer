...
qos_profile = QoSProfile(depth=1, reliability=ReliabilityPolicy.BEST_EFFORT, history=HistoryPolicy.KEEP_LAST)
...
class CameraPreProcessor(Node):
    def __init__(self):
        ...
        for camera in config["active_cameras"]:
            ...
            self.subs[f"{camera}_depth"] = (
                self.create_subscription(  # for depth cam
                    msg_type=Image,
    ...
    def info_callback(self, msg, camera):
        ...
        target = config["target_resolution"]
        new_K, _ = cv2.getOptimalNewCameraMatrix(K, D, (msg.width, msg.height), 0, (target[0], target[1]))
        map1, map2 = cv2.initUndistortRectifyMap(K, D, None, new_K, (target[0], target[1]), cv2.CV_16SC2)
